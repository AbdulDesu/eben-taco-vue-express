        ...
        <div v-if="filterBills.length > 0" class="my-order-cards">
            <div v-for="b in filterBills.slice().reverse()" class="card" :key="b.bill_id">
                <div class="card-head d-flex flex-wrap flex-sm-nowrap justify-content-between">
                    ...
                    <button @click="sendBillId(b.bill_id)">Tampilkan detail pesanan</button>
                </div>

                <div class="d-flex flex-wrap flex-sm-nowrap justify-content-between card-summary">
                    <div class="w-100 text-center py-1 px-2"><span>Status Pembayaran:</span>{{ " " + b.bill_paid }}
                    </div>
                    <div class="w-100 text-center py-1 px-2"><span>Status Pesanan:</span>{{ " " + avaiableStatus[b.bill_status]
                    }}
                    </div>
                    <div class="w-100 text-center py-1 px-2"><span>Tanggal Pemesanan:</span> {{ b.bill_when }}</div>
                </div>
                <div class="d-flex flex-wrap flex-sm-nowrap justify-content-between card-summary">

                    <div class="w-100 text-center py-1 px-2"><span>Total:</span> Rp. {{ b.bill_total }}</div>
                    <div class="w-100 text-center py-1 px-2"><span>Alamat:</span>{{ " " + b.bill_address }}
                    </div>
                    <div class="w-100 text-center py-1 px-2"><span>Nomor telepon:</span>{{ " " + b.bill_phone }}
                    </div>
                </div>
