        ...
        <div class="checkout-form-container">
            <form id="checkoutForm" @submit="handleSubmit" novalidate autocomplete="off">
                <div class="checkout-heading">
                    <h3>Selesaikan Pembayaran Anda<span>Total</span></h3>
                    <h3 v-if="user">Pesanan oleh {{ user.user_name }}<span>Rp. {{ calculateSummaryPrice()[3] }}</span></h3>
                </div>

                <div class="form-group details-group">
                    <h4>Alamat Pemesanan</h4>
                    <div class="form-group">
                        <input type="text" name="coPhone" id="coPhone" placeholder="Nomor Handphone" class="form-control"
                            v-model="checkoutObj.phone"/>
                        <p class="error-mess" v-if="errorObj.phoneErr.length > 0">{{ errorObj.phoneErr[0] }}</p>
                    </div>

                    <div class="form-group">
                        <input type="text" name="coAddress" id="coAddress" placeholder="Alamat Lengkap"
                ...
                <div class="form-group details-group">
                    ...
                    <div class="form-group">
                        <div class="form-group">
                            <input type="radio" name="payment" value="cash" id="paymentCash"
                                v-model="checkoutObj.paymentMethod" /><span>Tunai </span>
                    ...
                    <div v-if="checkoutObj.paymentMethod == 'card'">
                        <div class="form-group">
                            <input type="text" name="coCardNum" placeholder="Nomor Kartu" id="coCardNum"
                        ...
                        <div class="form-group">
                            <input v-upcase type="text" name="coCardName" placeholder="Nama Lengkap Di Kartu"
                        ...
                        <div class="form-group">
                            <div class="form-control">
                                <span
                                    style="font-size: 1.6rem; position: absolute; margin-left: -5px; margin-top: -11px;">Tanggal Kadaluarsa:
                                </span>
                                <input
    ...
    methods: {
        ...
        checkForm: function () {
            ...
            if (!this.checkoutObj.address) {
                this.errorObj.addressErr.push('Silahkan isi alamat lengkap anda');
            }

            if (!this.checkoutObj.paymentMethod) {
                this.errorObj.payErr.push('Silahkan pilih metode pembayaran anda');
            }
            else if (this.checkoutObj.paymentMethod == "card") {
                if (!this.cardObj.number) {
                    this.errorObj.numErr.push('Silahkan isi nomor kartu anda');
                }
                else {
                    if (!this.cardObj.number.startsWith('4')) {
                        this.errorObj.numErr.push('Nomor Kartu kartu visa dimulai dari angka 4');
                    }

                    if (this.cardObj.number.length != 16) {
                        this.errorObj.numErr.push('Nomor Kartu visa adalah 16 digit!');
                    }

                    if (!/[0-9]{16}/.test(this.cardObj.number)) {
                        this.errorObj.numErr.push('Nomor kartu visa tidak valid');
                    }
                }
                ...
                if (!this.cardObj.name) {
                    ...
                    if (!/^[A-Za-z]+$/.test(this.cardObj.name.replace(/\s/g, ""))) {
                        this.errorObj.nameErr.push('Nama tidak valid');
                    }
                }

                if (!this.cardObj.expiryDate) {
                    this.errorObj.exDateErr.push('Tanggal kadaluarsa kartu tidak valid');
                }


                if (!this.cardObj.cvv) {
                    this.errorObj.cvvErr.push('Silahkan isi CVV anda');
                }
                else {
                    if (this.cardObj.cvv.length != 3) {
                        this.errorObj.cvvErr.push('CVV harus 3 digit');
                    }

                    if (!/[0-9]{3}/.test(this.cardObj.cvv)) {
                        this.errorObj.cvvErr.push('CVV tidak valid');
                    }
                }
